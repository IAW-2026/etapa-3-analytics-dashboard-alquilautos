...
export function getCancelados(desde: string, hasta: string) {
...
export async function GET(req: NextRequest) {
  ...
  const result = await getCancelados(desde, hasta);
  if (result.error) return NextResponse.json({ error: result.error }, { status: 500 });
  return NextResponse.json({ cancelados: result.data?.cancelados });
}
