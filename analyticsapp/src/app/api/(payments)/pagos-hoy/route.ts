...
export function getPagosHoy(desde: string, hasta: string) {
...
export async function GET(req: NextRequest) {
  ...
  const result = await getPagosHoy(desde, hasta);
  if (result.error) return NextResponse.json({ error: result.error }, { status: 500 });
  return NextResponse.json({ pagos_hoy: result.data?.pagos_hoy });
}
