...
export function getPagosRealizados(desde: string, hasta: string) {
...
export async function GET(req: NextRequest) {
  ...
  const result = await getPagosRealizados(desde, hasta);
  if (result.error) return NextResponse.json({ error: result.error }, { status: 500 });
  return NextResponse.json({ pagos_totales: result.data?.pagos_totales });
}
